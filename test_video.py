...
# These will be set after parsing args based on --model_type
DMC = None
IntraNoAR = None
str2bool = None
create_folder = None
generate_log_json = None
dump_json = None
...
calc_psnr = None
calc_msssim = None
ycbcr444_to_420 = None
ycbcr420_to_444 = None


def setup_imports(model_type):
    """Setup imports based on model type."""
    global DMC, IntraNoAR, str2bool, create_folder, generate_log_json, dump_json
    global get_padding_size, get_state_dict, PNGReader, YUVReader, PNGWriter, YUVWriter
    global calc_psnr, calc_msssim, ycbcr444_to_420, ycbcr420_to_444

    # Get the base directory
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # Always use official for helpers, image model, etc.
    official_path = os.path.join(base_dir, 'model', 'official')
    if official_path not in sys.path:
        sys.path.insert(0, official_path)

    # Import all shared modules from official
    from src.models.image_model import IntraNoAR as _IntraNoAR
    from src.utils.common import str2bool as _str2bool, create_folder as _create_folder
    from src.utils.common import generate_log_json as _generate_log_json, dump_json as _dump_json
    from src.utils.stream_helper import get_padding_size as _get_padding_size, get_state_dict as _get_state_dict
    from src.utils.video_reader import PNGReader as _PNGReader, YUVReader as _YUVReader
    from src.utils.video_writer import PNGWriter as _PNGWriter, YUVWriter as _YUVWriter
    from src.utils.metrics import calc_psnr as _calc_psnr, calc_msssim as _calc_msssim
    from src.transforms.functional import ycbcr444_to_420 as _ycbcr444_to_420, ycbcr420_to_444 as _ycbcr420_to_444

    # Import the appropriate video model
    if model_type == 'official':
        from src.models.video_model import DMC as _DMC
    else:  # 'open_transform'
        # Add open path for video_model_transform
        open_path = os.path.join(base_dir, 'model', 'open')
        if open_path not in sys.path:
            sys.path.insert(0, open_path)
        from src.models.video_model_transform import DMC as _DMC

    # Assign to globals
    DMC = _DMC
    IntraNoAR = _IntraNoAR
    str2bool = _str2bool
    create_folder = _create_folder
    generate_log_json = _generate_log_json
    dump_json = _dump_json
    get_padding_size = _get_padding_size
    get_state_dict = _get_state_dict
    PNGReader = _PNGReader
    YUVReader = _YUVReader
    PNGWriter = _PNGWriter
    YUVWriter = _YUVWriter
    calc_psnr = _calc_psnr
    calc_msssim = _calc_msssim
    ycbcr444_to_420 = _ycbcr444_to_420
    ycbcr420_to_444 = _ycbcr420_to_444


def _str2bool_local(v):
    """Local str2bool for argument parsing before imports are set up."""
...
def parse_args():
    ...
    # Model type selection - must be parsed first
    parser.add_argument('--model_type', type=str, default='official',
                        choices=['official', 'open_transform'],
                        help="Model type to use: 'official' (original DCVC-DC) "
                             "or 'open_transform' (video_model_transform.py)")

    parser.add_argument("--ec_thread", type=_str2bool_local, nargs='?', const=True, default=False)
    ...
    parser.add_argument("--force_intra", type=_str2bool_local, nargs='?', const=True, default=False)
    ...
    parser.add_argument('--yuv420', type=_str2bool_local, default=False, required=False)
    parser.add_argument("--worker", "-w", type=int, default=1, help="worker number")
    parser.add_argument("--cuda", type=_str2bool_local, nargs='?', const=True, default=False)
    parser.add_argument("--cuda_device", default=None,
                        help="the cuda device used, e.g., 0; 0,1; 1,2,3; etc.")
    parser.add_argument('--calc_ssim', type=_str2bool_local, default=False, required=False)
    parser.add_argument('--write_stream', type=_str2bool_local, nargs='?',
                        const=True, default=False)
    parser.add_argument('--stream_path', type=str, default="out_bin")
    parser.add_argument('--save_decoded_frame', type=_str2bool_local, default=False)
...
def main():
    ...
    # Setup imports based on model type (for main process)
    setup_imports(args.model_type)
