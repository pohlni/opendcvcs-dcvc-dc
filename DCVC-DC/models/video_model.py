...
def check_nan(tensor, name):
    """Check for NaN/Inf in tensor and print debug info if found."""
    if tensor is None:
        return False
    has_nan = torch.isnan(tensor).any().item()
    has_inf = torch.isinf(tensor).any().item()
    if has_nan or has_inf:
        print(f"[NaN DEBUG] {name}: has_nan={has_nan}, has_inf={has_inf}, "
              f"min={tensor.min().item():.4f}, max={tensor.max().item():.4f}, "
              f"mean={tensor.mean().item():.4f}, shape={list(tensor.shape)}", flush=True)
        return True
    return False
...
class DMC(CompressionModel):
    ...
    def encode_decode(self, x, dpb, q_in_ckpt, q_index, output_path=None,
        ...
        result = {
            ...
            "encoding_time": 0,
            "decoding_time": 0,
            "bits_y_map": encoded.get('bits_y_map'),
            "bits_mv_y_map": encoded.get('bits_mv_y_map'),
            "bits_z_map": encoded.get('bits_z_map'),
            "bits_mv_z_map": encoded.get('bits_mv_z_map'),
        }
        return result

    def forward_one_frame(self, x, dpb, q_in_ckpt=False, q_index=None, frame_idx=0,stage=1,lmbda=1):
        ...
        check_nan(x, "x (input)")
        check_nan(dpb["ref_frame"], "dpb[ref_frame]")

        est_mv = self.optic_flow(x, dpb["ref_frame"])
        check_nan(est_mv, "est_mv (after optic_flow)")

        mv_y = self.mv_encoder(est_mv, dpb["ref_mv_feature"], mv_y_q_enc)
        check_nan(mv_y, "mv_y (after mv_encoder)")

        mv_y_pad, slice_shape = self.pad_for_y(mv_y)
        mv_z = self.mv_hyper_prior_encoder(mv_y_pad)
        mv_z_hat = self.quant(mv_z)
        mv_params = self.mv_prior_param_decoder(mv_z_hat, dpb, slice_shape)
        check_nan(mv_params, "mv_params (after mv_prior_param_decoder)")
        mv_y_res, mv_y_q, mv_y_hat, mv_scales_hat = self.forward_four_part_prior(
            ...
            self.mv_y_spatial_prior_adaptor_3, self.mv_y_spatial_prior)
        check_nan(mv_scales_hat, "mv_scales_hat (after forward_four_part_prior)")
        ...
        y = self.contextual_encoder(x, context1, context2, context3, y_q_enc)
        check_nan(y, "y (after contextual_encoder)")

        y_pad, slice_shape = self.pad_for_y(y)
        z = self.contextual_hyper_prior_encoder(y_pad)
        check_nan(z, "z (after hyper_prior_encoder)")

        z_hat = self.quant(z)
        params = self.res_prior_param_decoder(z_hat, dpb, context3, slice_shape)
        check_nan(params, "params (after res_prior_param_decoder)")

        y_res, y_q, y_hat, scales_hat = self.forward_four_part_prior(
            y, params, self.y_spatial_prior_adaptor_1, self.y_spatial_prior_adaptor_2,
            self.y_spatial_prior_adaptor_3, self.y_spatial_prior)
        check_nan(scales_hat, "scales_hat (after forward_four_part_prior, raw)")
        ...
        check_nan(scales_hat, "scales_hat (after softplus1)")
        scales_hat = 5.55 - torch.nn.functional.softplus(5.55 - scales_hat)  # make logscale < 5.55
        check_nan(scales_hat, "scales_hat (after softplus2)")
        scales_hat = torch.exp(scales_hat)
        check_nan(scales_hat, "scales_hat (after exp)")
        ...
        batch_size = x.size(0)
        bpp_y = torch.sum(bits_y) / (pixel_num * batch_size)
        bpp_z = torch.sum(bits_z) / (pixel_num * batch_size)
        bpp_mv_y = torch.sum(bits_mv_y) / (pixel_num * batch_size)
        bpp_mv_z = torch.sum(bits_mv_z) / (pixel_num * batch_size)
        ...
        return {"bpp_mv_y": bpp_mv_y,
                ...
                "bit_mv_z": bit_mv_z,
                "bits_y_map": bits_y,
                "bits_mv_y_map": bits_mv_y,
                "bits_z_map": bits_z,
                "bits_mv_z_map": bits_mv_z,
                }
